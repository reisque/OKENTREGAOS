...
const formatNextConsultation = (value: string) => {
  const date = new Date(value)
  if (Number.isNaN(date.getTime())) return value
  date.setMinutes(date.getMinutes() + 30)
  return formatDate(date.toISOString())
}
const validDateValue = (value: string) => /^\d{4}-\d{2}-\d{2}$/.test(value) ? value : ''
...
export default function App() {
  ...
  return <main className="page">
    ...
    <section className="panel"><div className="sync-bar"><span>{consultedAt ? <><span>U: {formatDate(consultedAt)}</span><span>P: {formatNextConsultation(consultedAt)}</span></> : loading ? 'Carregando última atualização...' : 'Nenhuma consulta salva.'}</span><button type="button" onClick={() => void sync()} disabled={loading}><RefreshCw size={16} className={loading ? 'spin' : ''} />{loading ? 'Atualizando...' : 'Atualizar consulta'}</button></div><label htmlFor="batch-search">Filtrar várias OS de uma vez</label><div className="batch-query"><textarea id="batch-search" value={batchQuery} onChange={event => setBatchQuery(event.target.value)} placeholder={'Cole uma OS por linha, por exemplo:\n6SP 558788B\n6PE 413251B'} rows={3} /><div className="batch-actions"><button type="button" onClick={applyBatchFilter} disabled={!batchQuery.trim()}>Filtrar OS</button>{batchFilter.length > 0 && <button type="button" className="secondary" onClick={clearBatchFilter}>Limpar filtro</button>}</div></div><div className="date-filters"><div><label htmlFor="integration-start">Data integração - início</label><input id="integration-start" type="date" max="9999-12-31" value={integrationStart} onChange={event => { setIntegrationStart(validDateValue(event.target.value)); setPage(1) }} /></div><div><label htmlFor="integration-end">Data integração - fim</label><input id="integration-end" type="date" max="9999-12-31" value={integrationEnd} onChange={event => { setIntegrationEnd(validDateValue(event.target.value)); setPage(1) }} /></div><div><label htmlFor="cte-start">Data do CTE - início</label><input id="cte-start" type="date" max="9999-12-31" value={cteStart} onChange={event => { setCteStart(validDateValue(event.target.value)); setPage(1) }} /></div><div><label htmlFor="cte-end">Data do CTE - fim</label><input id="cte-end" type="date" max="9999-12-31" value={cteEnd} onChange={event => { setCteEnd(validDateValue(event.target.value)); setPage(1) }} /></div><button type="button" className="clear-date-filters" onClick={() => { clearDateFilters(); setPage(1) }} disabled={!integrationStart && !integrationEnd && !cteStart && !cteEnd}>Limpar datas</button></div><label htmlFor="search">Pesquisar nos resultados</label><div className="query"><div className="search-input"><Search size={18} /><input id="search" value={query} onChange={event => { setQuery(event.target.value); setPage(1) }} placeholder="Pesquise OS, status, booking, contêiner, contratante ou depot" /></div></div>{error && <div className="error">{error}</div>}</section>
    {consultedAt && <section className="results"><div className="results-head"><div><h2>Resultado da consulta</h2><span>{visibleResults.length} exibida(s) de {foundCount} encontrada(s)</span></div></div>{!results.length ? <p className="empty">Nenhuma OS foi retornada pelo portal para o ano de {new Date(consultedAt).getFullYear()}.</p> : <><div className="table-scroll"><table><thead><tr><th>Ordem de serviço</th><th>Data integração</th><th>Status</th><th>Data do CTE</th><th>Booking</th><th>Contêiner</th><th>Contratante</th><th>Depot</th><th>XML</th></tr></thead><tbody>{pageResults.map((item, index) => item.found
